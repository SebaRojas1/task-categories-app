<template>
    <div class="p-4">
      ...
      <div v-if="tasks.length">
        <div class="flex justify-between mb-4">
          <span class="text-sm text-gray-500">{{ tasksCompleted }} de {{ tasks.length }} completadas</span>
          <button
      ...
      <!-- Edit modal -->
      <div v-if="editingTask" class="fixed inset-0 bg-black/30 backdrop-blur-sm flex items-center justify-center z-50">
  ...
  async function fetchTasks() {
    try {
      const response = await axiosClient.get(`/categories/${props.categoryId}/tasks`)
  ...
  async function addTask() {
    ...
    try {
      const response = await axiosClient.post(`/categories/${props.categoryId}/tasks`, {
  ...
  async function saveEditedTask() {
    ...
    try {
      ...
      const index = tasks.value.findIndex(task => task._id === editingTask.value._id)
      if (index !== -1) {
