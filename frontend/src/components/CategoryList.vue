<template>
  <div class="max-w-4xl mx-auto">
    ...
    <div v-if="categories.length" class="space-y-6">
      ...
      >
          ...
          <div class="flex items-center">
            ...
            <div>
              ...
              <p class="text-sm text-gray-500">
                {{ category.tasks ? `${category.tasks.length} tareas` : '0 tareas' }}
              </p>
            </div>
          </div>
        ...
        <div v-if="openCategories.includes(category._id)" class="border-t">
          <TaskList :categoryId="category._id" :categoryName="category.name" />
        </div>
      </div>
    </div>
    ...
    <div v-else class="text-center py-12 bg-white rounded-lg shadow-md">
      ...
      <p class="text-gray-400 text-sm mt-2">Crea una categoría para organizar tus tareas.</p>
    </div>
    
    <!-- Edit modal -->
    <div v-if="editingCategory" class="fixed inset-0 bg-black/30 backdrop-blur-sm flex items-center justify-center z-50">
...
const handleDelete = async (id) => {
  const confirmDelete = confirm('¿Estás seguro de que deseas eliminar esta categoría y todas sus tareas?')
